(function(global, undefined) {
  ...
  colour.saturate = function(value) {
    var ca = this._raw.CIELab[1];
    var cb = this._raw.CIELab[2];
    var length = Math.pow(Math.pow(ca, 2) + Math.pow(cb, 2), 1 / 2);
    var e = [ca / length, cb / length];

    var L = this._raw.CIELab[0];
    var a = this._raw.CIELab[1] + e[0] * value * 100;
    var b = this._raw.CIELab[2] + e[1] * value * 100;

    var lab = createCIELab([L, a, b]);
    return createColour(lab);
  };

  colour.desaturate = function(value) {
    var ca = this._raw.CIELab[1];
    var cb = this._raw.CIELab[2];
    var length = Math.pow(Math.pow(ca, 2) + Math.pow(cb, 2), 1 / 2);
    var e = [ca / length, cb / length];
